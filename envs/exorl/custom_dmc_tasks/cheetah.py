...
def make(
    task, task_kwargs=None, environment_kwargs=None, visualize_reward: bool = False
):
...
def walk(time_limit: int = _DEFAULT_TIME_LIMIT, random=None, environment_kwargs=None):
...
@SUITE.add("benchmarking")
def walk_backward(
    time_limit: int = _DEFAULT_TIME_LIMIT, random=None, environment_kwargs=None
):
    """Returns the run task."""
    physics = Physics.from_xml_string(*get_model_and_assets())
...
class Cheetah(base.Task):
    ...
    def __init__(
        self, move_speed=_RUN_SPEED, forward=True, flip=False, random=None
    ) -> None:
