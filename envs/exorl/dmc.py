# based on https://github.com/kvfrans/cfgrl/blob/main/rlbase/envs/exorl/dmc.py
...
from dm_env import Environment, StepType, specs
...
class EnvWrapper:
    ...
    def _augment_time_step(
        self, time_step: TimeStep, action: tp.Optional[np.ndarray] = None
    ) -> TimeStep:
...
class FlattenJacoObservationWrapper(EnvWrapper):
    def __init__(self, env: Env) -> None:
        ...
        dim = np.sum(
            np.fromiter(
                (
                    int(np.prod(spec.shape))  # type: ignore
                    for spec in wrapped_obs_spec.values()
                ),
                np.int32,
            )
...
def _make_jaco(
    obs_type, domain, task, frame_stack, action_repeat, seed
...
class FloatStats:
    ...
    def add(self, value: float) -> "FloatStats":
        ...
        self.mean = (
            self._count - 1
        ) / self._count * self.mean + 1 / self._count * value
        return self
...
class DMCWrapper(core.Env):
    def __init__(
        ...
        if from_pixels:
            ...
            self._observation_space = _spec_to_box(
                [self._env.observation_spec()], np.float32
            )
    ...
    def step(self, action):
        ...
        # Check for NaN or Inf values which cause physics errors
        if not np.isfinite(action).all():
            print(f"ERROR: Action contains NaN or Inf! Action: {action}")
            print(
                f"  Has NaN: {np.isnan(action).any()}, Has Inf: {np.isinf(action).any()}"
            )
            # Replace NaN/Inf with zeros as a fallback
            action = np.nan_to_num(action, nan=0.0, posinf=1.0, neginf=-1.0)
            print(f"  Replaced with: {action}")

        # Check if action is out of bounds and print for debugging
        if not self._norm_action_space.contains(action):
            print(f"WARNING: Action out of bounds! Action: {action}")
            print(f"  Min: {action.min():.6f}, Max: {action.max():.6f}")
            print(
                f"  Expected range: [{self._norm_action_space.low[0]:.6f}, {self._norm_action_space.high[0]:.6f}]"
            )

        # Clip actions to valid range (handles floating point precision issues)
        action = np.clip(
            action, self._norm_action_space.low, self._norm_action_space.high
        )
        action = self._convert_action(action)
        action = np.clip(
            action, self._true_action_space.low, self._true_action_space.high
        )
    ...
    def reset(self, seed=None, options=None):
        # Gymnasium API: reset accepts seed and options
        if seed is not None:
            self.seed(seed)
        time_step = self._env.reset()
        self.current_state = _flatten_obs(time_step.observation)
